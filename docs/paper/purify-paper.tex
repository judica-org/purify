...
It also fixes generators
\[
G_i \in E_i(\mathbb{F}_P)
\]
and hash-to-curve maps
\[
H_i : \{0,1\}^* \to E_i(\mathbb{F}_P)
\]
for $i \in \{1,2\}$. We write
\[
...
\]
\[
[z_1]M_1 \in E_1(\mathbb{F}_P), \qquad [z_2]M_2 \in E_2(\mathbb{F}_P),
\]
\[
u = X([z_1]M_1), \qquad v = X([z_2]M_2) \in \mathbb{F}_P,
\]
\[
...
\[
P_{1,x} = X([z_1]G_1), \qquad P_{2,x} = X([z_2]G_2),
\]
\[
Q_{1,x} = X([z_1]M_1), \qquad Q_{2,x} = X([z_2]M_2),
\]
\[
...
\[
P_1 = [z_1]G_1, \qquad P_2 = [z_2]G_2,
\]
\[
Q_1 = [z_1]M_1, \qquad Q_2 = [z_2]M_2,
\]
...
\[
P_{1,x} = \texttt{MulX}(G_1,z_1), \qquad
P_{2,x} = \texttt{MulX}(G_2,z_2),
\]
\[
Q_{1,x} = \texttt{MulX}(M_1,z_1), \qquad
Q_{2,x} = \texttt{MulX}(M_2,z_2).
\]
...
\]
The key point is that the native circuit has \emph{two} kinds of checks, not one:
\begin{enumerate}
  \item Every recorded multiplicative constraint becomes one numbered gate with wire symbols
  ...
  a_{L,i} a_{R,i} = a_{O,i}.
  \]
  \item Every recorded affine equality becomes one sparse linear row over those gate wires and over
...
The second layer is where the row equation comes from. These ``second kind'' checks are just the
...
\[
A_I = \mathrm{Com}(a_L,a_R;\alpha),\qquad
A_O = \mathrm{Com}(a_O;\beta),\qquad
