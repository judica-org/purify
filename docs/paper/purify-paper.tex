...
\begin{enumerate}[nosep]
  ...
  (referred to here as the BP++ path; the repository also uses the names \texttt{bppp}/BPP)~\cite{bulletproofs-plus-plus,secp-zkp-main,secp-bppp}.
\end{enumerate}
...
the later slack tail in the reduced vector $\ell$. For the current Purify parameters the unpadded
...
o = V_0,
\]
where $V_0$ is the single committed-scalar slot of the Bulletproof statement. The BP proof is therefore a
...
BP++ is therefore not a different Purify statement. It is a public reduction of the same Purify circuit to
...
about the norm-vector dimension $4N$; it does not require removing the separate slack tail later
...
\]
where the final tail is reduction slack: coordinates introduced by rounding the $\ell$-vector
length up to a power of two after the used coordinates have been placed. This is distinct from the
earlier gate-count padding of the native circuit. In the basic reduction these slack coordinates
are zero; in the masked wrapper they are reused as blinding slots. The extra ``$+1$'' in the
...
output-wire part, the next $C$ entries weight the commitment-wire part, and the slack tail is zero.
...
\sum_{k=0}^{4N-1} \mu_k n_k^2 + \langle \ell, c \rangle = T
\]
under that commitment. This framing makes the BP++ design easiest to read: it proves the same Purify
...
proof. Starting from the hidden vectors $(n,\ell)$, the prover first fills the reduction-slack tail
of $\ell$ with fresh blind scalars and then derives random mask vectors
\[
...
commitment. The outer protocol is Bulletproof-like, while the inner statement remains the BP++
