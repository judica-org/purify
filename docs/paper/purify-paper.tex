...
\newcommand{\bp}{\textbf{BulletProofs}}
\newcommand{\bppp}{\textbf{BulletProofs++}}
...
The mathematical output is therefore a field element $o \in \mathbb{F}_P$. The proof systems used
later do not always reveal that scalar directly. In the public-message setting studied in this
...
\]
That means the same byte string can be read directly as a secp256k1 scalar. Except for the
single invalid value $r_0 = 0$, the Purify output can be used as a Schnorr nonce scalar without any
...
R = [r]G_{\mathrm{secp}}.
\]
has even $Y$ and the same x-only encoding as $R_0$. This is exactly the normalization performed by
\texttt{purify\_bip340\_nonce\_from\_scalar(...)}, which rewrites the nonce scalar in place to the
even-$Y$ representative corresponding to the returned x-only nonce. In the API this is exposed in
x-only form, because that is the BIP340 nonce encoding. In the
...
R = [r]G_{\mathrm{secp}}.
\]
The later proof systems are then used to show that the published x-only nonce matches the
...
\texttt{hash\_to\_curve} before any circuit logic is created. Hash-to-curve is therefore outside
the proved statement. The proved relation begins after the message has already been turned into the public curve
points $M_1$ and $M_2$.
...
This is where the message-specific structure matters. Since the base point is public, the required
point tables can be precomputed \emph{outside} the circuit:
\begin{itemize}[nosep]
...
The later affine-addition helpers are not invoked on arbitrary pairs of unknown points. In the
reference construction they are only called on affine points that are guaranteed by construction not
to be equal and not to be negatives of each other, so the denominator $x_2-x_1$ is non-zero on
...
Division is handled indirectly because the native constraint system has no inverse gate. Instead,
...
helpers are only reached on point pairs guaranteed by construction not to be equal or negatives of
each other, and the final combine step likewise requires $u \neq v/D$ on the concrete witness. The
transcript language does not prove those non-vanishing facts by itself; it relies on the circuit
...
Within that vocabulary, all of the following are represented by transcript operations:
\begin{itemize}[nosep]
...
\texttt{div}. The final Purify formula is therefore not special-cased at the backend layer; it is just
...
\end{enumerate}
After that, the number of multiplication gates in the native circuit is padded to the next power
...
circuit is about 2030 multiplication gates, and the proof-facing shape is padded to 2048 gates.
...
The key point is the Purify output $o$. In the protocol, $o$ is supposed to remain secret:
...
o = V_0,
\]
where $V_0$ is the single committed-scalar slot of the \bp\ statement. The \bp\ proof is therefore a
...
an extra blinding factor for this commitment. If a blind is provided, the backend uses a blinded
...
The \bp\ path is therefore the most literal reading of the repository:
\begin{quote}
...
\bppp\ is therefore not a different Purify statement. It is a public reduction of the same Purify circuit to
...
\]
where the final tail consists of zk slack variable slots: coordinates introduced by rounding the $\ell$-vector
length up to a power of two after the used coordinates have been placed. This is distinct from the
earlier gate-count padding of the native circuit. In the basic reduction these zk slack variable
slots are zero; in the masked wrapper they are reused as blinding slots. The extra ``$+1$'' in the
...
\sum_{k=0}^{4N-1} \mu_k n_k^2 + \langle \ell, c \rangle = T,
\]
with $\mu_k = (\rho^2)^{k+1}$. This is the object the \bppp\ backend is designed to prove.
...
\begin{enumerate}[nosep]
  \item The \bppp\ interfaces in this repository are marked experimental. They are useful for
