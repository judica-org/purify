...
purify::puresign::PublicKey to_legacy_public_key(const PublicKey& public_key) {
    return purify::puresign::PublicKey{public_key.purify_pubkey, public_key.bip340_pubkey};
}

PublicKey from_legacy_public_key(const purify::puresign::PublicKey& public_key) {
    PublicKey out{};
    out.purify_pubkey = public_key.purify_pubkey;
    out.bip340_pubkey = public_key.bip340_pubkey;
    return out;
}
...
Result<PublicKey> derive_public_key(const UInt512& secret) {
    Result<purify::puresign::PublicKey> public_key = purify::puresign::PublicKey::from_secret(secret);
    if (!public_key.has_value()) {
        return unexpected_error(public_key.error(), "derive_public_key:from_secret");
    }
    return from_legacy_public_key(*public_key);
}
...
Result<PreparedNonce> prepare_message_nonce(const UInt512& secret, std::span<const unsigned char> message) {
    ...
    if (!key_pair.has_value()) {
        return unexpected_error(key_pair.error(), "prepare_message_nonce:from_secret");
    }
    return key_pair->prepare_message_nonce(message);
}
...
Result<PreparedNonceWithProof> prepare_message_nonce_with_proof(const UInt512& secret,
    ...
    Result<purify::puresign::KeyPair> key_pair = purify::puresign::KeyPair::from_secret(secret);
    if (!key_pair.has_value()) {
        return unexpected_error(key_pair.error(), "prepare_message_nonce_with_proof:from_secret");
    }
    Result<PreparedNonce> prepared = key_pair->prepare_message_nonce(message);
...
Result<PreparedNonceWithProof> prepare_message_nonce_with_proof(const UInt512& secret,
    ...
    Result<PreparedNonce> prepared = key_pair->prepare_message_nonce(cache.message);
    if (!prepared.has_value()) {
        return unexpected_error(prepared.error(), "prepare_message_nonce_with_proof:prepare_message_nonce");
    }
...
Result<PreparedNonce> prepare_topic_nonce(const UInt512& secret, std::span<const unsigned char> topic) {
    Result<purify::puresign::KeyPair> key_pair = purify::puresign::KeyPair::from_secret(secret);
    if (!key_pair.has_value()) {
        return unexpected_error(key_pair.error(), "prepare_topic_nonce:from_secret");
    }
    return key_pair->prepare_topic_nonce(topic);
}
...
Result<PreparedNonceWithProof> prepare_topic_nonce_with_proof(const UInt512& secret,
    ...
    Result<purify::puresign::KeyPair> key_pair = purify::puresign::KeyPair::from_secret(secret);
    if (!key_pair.has_value()) {
        return unexpected_error(key_pair.error(), "prepare_topic_nonce_with_proof:from_secret");
    }
    Result<PreparedNonce> prepared = key_pair->prepare_topic_nonce(topic);
    if (!prepared.has_value()) {
...
Result<PreparedNonceWithProof> prepare_topic_nonce_with_proof(const UInt512& secret,
    ...
    Result<PreparedNonce> prepared = key_pair->prepare_topic_nonce(cache.topic);
    if (!prepared.has_value()) {
        return unexpected_error(prepared.error(), "prepare_topic_nonce_with_proof:prepare_topic_nonce");
    }
    Result<NonceProof> proof =
...
Result<Signature> sign_message(const UInt512& secret, std::span<const unsigned char> message) {
    Result<purify::puresign::KeyPair> key_pair = purify::puresign::KeyPair::from_secret(secret);
    if (!key_pair.has_value()) {
        return unexpected_error(key_pair.error(), "sign_message:from_secret");
    }
    return key_pair->sign_message(message);
}
...
Result<Signature> sign_message_with_prepared(const UInt512& secret, std::span<const unsigned char> message,
    ...
    Result<purify::puresign::KeyPair> key_pair = purify::puresign::KeyPair::from_secret(secret);
    if (!key_pair.has_value()) {
        return unexpected_error(key_pair.error(), "sign_message_with_prepared:from_secret");
    }
    return key_pair->sign_message_with_prepared(message, std::move(prepared));
}
...
Result<Signature> sign_with_topic(const UInt512& secret, std::span<const unsigned char> message,
    ...
    Result<purify::puresign::KeyPair> key_pair = purify::puresign::KeyPair::from_secret(secret);
    if (!key_pair.has_value()) {
        return unexpected_error(key_pair.error(), "sign_with_topic:from_secret");
    }
    return key_pair->sign_with_topic(message, topic);
}
...
Result<Signature> sign_with_prepared_topic(const UInt512& secret, std::span<const unsigned char> message,
    ...
    Result<purify::puresign::KeyPair> key_pair = purify::puresign::KeyPair::from_secret(secret);
    if (!key_pair.has_value()) {
        return unexpected_error(key_pair.error(), "sign_with_prepared_topic:from_secret");
    }
    return key_pair->sign_with_prepared_topic(message, std::move(prepared));
}
...
Result<bool> verify_signature(const PublicKey& public_key, std::span<const unsigned char> message,
    ...
    return to_legacy_public_key(public_key).verify_signature(message, signature);
}
...
Bytes PublicKey::serialize() const {
    return to_legacy_public_key(*this).serialize();
}

Result<PublicKey> PublicKey::deserialize(std::span<const unsigned char> serialized) {
    Result<purify::puresign::PublicKey> public_key = purify::puresign::PublicKey::deserialize(serialized);
    if (!public_key.has_value()) {
        return unexpected_error(public_key.error(), "PublicKey::deserialize:deserialize");
    }
    return from_legacy_public_key(*public_key);
}
