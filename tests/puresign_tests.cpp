...
void test_puresign_message_signing(TestContext& ctx) {
    ...
    Result<purify::puresign::KeyPair> key_pair = purify::puresign::KeyPair::from_secret(*secret);
    ...
    Result<purify::puresign::PreparedNonceWithProof> prepared_with_proof =
        key_pair->prepare_message_nonce_with_proof(message);
    expect_ok(ctx, prepared_with_proof, "KeyPair::prepare_message_nonce_with_proof succeeds");
    Result<purify::puresign::PreparedNonce> prepared_a = key_pair->prepare_message_nonce(message);
    expect_ok(ctx, prepared_a, "KeyPair::prepare_message_nonce succeeds");
    Result<purify::puresign::PreparedNonce> prepared_b = key_pair->prepare_message_nonce(message);
    ...
    Result<bool> nonce_proof_ok =
        public_key.verify_message_nonce_proof(message, prepared_with_proof->proof());
    ...
    Result<bool> wrong_nonce_proof =
        public_key.verify_message_nonce_proof(Bytes{0x89}, prepared_with_proof->proof());
    ...
    Result<purify::puresign::PreparedNonceWithProof> cached_prepared_with_proof =
        key_pair->prepare_message_nonce_with_proof(*proof_cache);
    ...
    if (cached_prepared_with_proof.has_value()) {
        ...
        Result<bool> cached_nonce_proof_ok =
            public_key.verify_message_nonce_proof(*proof_cache, cached_prepared_with_proof->proof());
    ...
    Result<purify::puresign::Signature> cached =
        key_pair->sign_message_with_prepared(message, std::move(*prepared_a));
    ...
    Result<purify::puresign::ProvenSignature> proven =
        key_pair->sign_message_with_prepared_proof(message, std::move(*prepared_with_proof));
    ...
    Result<purify::puresign::ProvenSignature> cached_proven =
        key_pair->sign_message_with_proof(*proof_cache);
    expect_ok(ctx, cached_proven, "KeyPair::sign_message_with_proof succeeds with a cached template");
    if (proven.has_value()) {
        Result<bool> proven_ok = public_key.verify_message_signature_with_proof(message, *proven);
        ...
        if (proven_bytes.has_value()) {
            ...
            if (parsed_proven.has_value()) {
                Result<bool> parsed_ok =
                    public_key.verify_message_signature_with_proof(message, *parsed_proven);
    ...
    if (cached_proven.has_value()) {
        Result<bool> cached_proven_ok = public_key.verify_message_signature_with_proof(*proof_cache, *cached_proven);
    ...
    Result<bool> verified = public_key.verify_signature(message, *direct);
    ...
    if (parsed_public_key.has_value() && parsed_signature.has_value()) {
        Result<bool> reparsed_verified = parsed_public_key->verify_signature(message, *parsed_signature);
...
void test_puresign_topic_signing(TestContext& ctx) {
    ...
    Result<purify::puresign::PreparedNonceWithProof> prepared_with_proof =
        key_pair->prepare_topic_nonce_with_proof(topic);
    expect_ok(ctx, prepared_with_proof, "KeyPair::prepare_topic_nonce_with_proof succeeds");
    Result<purify::puresign::PreparedNonce> prepared_a = key_pair->prepare_topic_nonce(topic);
    expect_ok(ctx, prepared_a, "KeyPair::prepare_topic_nonce succeeds");
    Result<purify::puresign::PreparedNonce> prepared_b = key_pair->prepare_topic_nonce(topic);
    ...
    Result<bool> nonce_proof_ok =
        public_key.verify_topic_nonce_proof(topic, prepared_with_proof->proof());
    ...
    Result<purify::puresign::PreparedNonceWithProof> cached_prepared_with_proof =
        key_pair->prepare_topic_nonce_with_proof(*proof_cache);
    ...
    if (cached_prepared_with_proof.has_value()) {
        ...
        Result<bool> cached_nonce_proof_ok =
            public_key.verify_topic_nonce_proof(*proof_cache, cached_prepared_with_proof->proof());
    ...
    Result<purify::puresign::Signature> direct = key_pair->sign_with_topic(message, topic);
    expect_ok(ctx, direct, "KeyPair::sign_with_topic succeeds");
    Result<purify::puresign::Signature> cached =
        key_pair->sign_with_prepared_topic(message, std::move(*prepared_a));
    ...
    Result<purify::puresign::ProvenSignature> proven =
        key_pair->sign_with_prepared_topic_proof(message, std::move(*prepared_with_proof));
    expect_ok(ctx, proven, "KeyPair::sign_with_prepared_topic_proof succeeds");
    Result<purify::puresign::ProvenSignature> cached_proven =
        key_pair->sign_with_topic_proof(message, *proof_cache);
    expect_ok(ctx, cached_proven, "KeyPair::sign_with_topic_proof succeeds with a cached template");
    if (proven.has_value()) {
        Result<bool> proven_ok = public_key.verify_topic_signature_with_proof(message, topic, *proven);
    ...
    if (cached_proven.has_value()) {
        Result<bool> cached_proven_ok = public_key.verify_topic_signature_with_proof(*proof_cache, message, *cached_proven);
    ...
    expect_error(ctx, key_pair->prepare_topic_nonce(Bytes{}), ErrorCode::EmptyInput,
...
void test_puresign_binding_checks(TestContext& ctx) {
    ...
    Result<purify::puresign::PreparedNonce> message_nonce = key_pair->prepare_message_nonce(message);
    expect_ok(ctx, message_nonce, "KeyPair::prepare_message_nonce succeeds for binding checks");
    if (message_nonce.has_value()) {
        expect_error(ctx, key_pair->sign_message_with_prepared(wrong_message, std::move(*message_nonce)),
                     ErrorCode::BindingMismatch,
                     "message-bound prepared nonces reject signing a different message");
    }

    Result<purify::puresign::PreparedNonce> topic_nonce = key_pair->prepare_topic_nonce(topic);
    expect_ok(ctx, topic_nonce, "KeyPair::prepare_topic_nonce succeeds for binding checks");
    if (topic_nonce.has_value()) {
        expect_error(ctx, key_pair->sign_message_with_prepared(message, std::move(*topic_nonce)),
                     ErrorCode::BindingMismatch,
...
void test_puresign_plusplus_message_signing(TestContext& ctx) {
    ...
    Result<purify::puresign_plusplus::KeyPair> key_pair = purify::puresign_plusplus::KeyPair::from_secret(*secret);
