...
bool proof_rejected_after_tamper(const NativeBulletproofCircuit& circuit,
    ...
    Result<bool> verified =
        purify::verify_experimental_circuit(circuit, *parsed, purify::bppp::base_generator(secp_context),
...
bool proven_signature_rejected_after_tamper(const purify::puresign::PublicKey& public_key,
    ...
    Result<Bytes> serialized = signature.serialize(secp_context);
    if (!serialized.has_value() || serialized->empty()) {
        return false;
    }
    Bytes tampered = *serialized;
    tampered[tampered.size() - 1] ^= 0x40;
    Result<purify::puresign::ProvenSignature> parsed =
        purify::puresign::ProvenSignature::deserialize(tampered, secp_context);
    if (!parsed.has_value()) {
        return true;
    }
    Result<bool> verified = public_key.verify_message_signature_with_proof(message, *parsed, secp_context);
    return !verified.has_value() || !*verified;
}
...
bool proven_signature_plusplus_rejected_after_tamper(const purify::puresign_plusplus::PublicKey& public_key,
    ...
    Result<purify::puresign_plusplus::ProvenSignature> parsed =
        purify::puresign_plusplus::ProvenSignature::deserialize(tampered, secp_context);
    if (!parsed.has_value()) {
        return true;
    }
    Result<bool> verified = public_key.verify_message_signature_with_proof(message, *parsed, secp_context);
...
void test_seeded_key_and_circuit_properties(TestContext& ctx) {
    ...
    for (std::size_t i = 0; i < 12; ++i) {
        ...
        Result<purify::Bip340Key> bip340 = purify::derive_bip340_key(first->secret, context.get());
        expect_ok(ctx, bip340, "derive_bip340_key succeeds on a generated secret");

        Result<purify::puresign::PublicKey> public_key =
            purify::puresign::PublicKey::from_secret(first->secret, context.get());
...
void test_random_experimental_proof_properties(TestContext& ctx) {
    ...
    Result<ExperimentalBulletproofProof> proof =
        purify::prove_experimental_circuit(*circuit, witness->assignment, nonce,
                                           purify::bppp::base_generator(context.get()), context.get(), binding);
    ...
    ctx.expect(proof_rejected_after_tamper(*circuit, *proof, context.get(), binding),
    ...
    for (std::size_t i = 0; i < 2; ++i) {
        ...
        Result<ExperimentalBulletproofProof> skipped =
            purify::prove_experimental_circuit_assume_valid(*circuit, witness->assignment, nonce,
                                                            purify::bppp::base_generator(context.get()),
                                                            context.get(), binding);
...
void test_random_puresign_proven_signature_properties(TestContext& ctx) {
    ...
    Result<purify::puresign::KeyPair> key_pair =
        purify::puresign::KeyPair::from_secret(key->secret, context.get());
    ...
    Result<purify::puresign::ProvenSignature> proven = key_pair->sign_message_with_proof(message, context.get());
    ...
    Result<bool> verified = public_key.verify_message_signature_with_proof(message, *proven, context.get());
    ...
    Result<bool> wrong_message =
        public_key.verify_message_signature_with_proof(random_bytes(rng, 1, 24), *proven, context.get());
    ...
    ctx.expect(proven_signature_rejected_after_tamper(public_key, message, context.get(), *proven),
...
void test_random_puresign_plusplus_proven_signature_properties(TestContext& ctx) {
    ...
    Result<purify::puresign_plusplus::KeyPair> key_pair =
        purify::puresign_plusplus::KeyPair::from_secret(key->secret, context.get());
    ...
    Result<purify::puresign_plusplus::ProvenSignature> proven =
        key_pair->sign_message_with_proof(message, context.get());
    ...
    ctx.expect(proven_signature_plusplus_rejected_after_tamper(public_key, message, context.get(), *proven),
